...
def TrajectoryGenerator(Tse_initial, Tsc_initial, Tsc_final, Tce_grasp, Tce_standoff, tf, k):
    csv_list = []
    ...
    traj1 = mr.ScrewTrajectory(Tse_initial, Tse_standoff, tf, tf*k/0.01, 5)
    csv_list = BuildcsvList(traj1, 0, csv_list)

    # STANDOFF ABOVE BLOCK DOWN TO BLOCK
    Tse_grasp = Tsc_initial@Tce_grasp
    traj2 = mr.ScrewTrajectory(Tse_standoff, Tse_grasp, tf, tf*k/0.01, 5)
    csv_list = BuildcsvList(traj2, 0, csv_list)

    # GRAB BLOCK
    traj3 = mr.ScrewTrajectory(Tse_grasp, Tse_grasp, int(tf/2), int((tf*k)/(2*0.01)), 5)
    csv_list = BuildcsvList(traj3, 1, csv_list)

    # MOVE BACK TO STANDOFF POSITION
    traj4 = mr.ScrewTrajectory(Tse_grasp, Tse_standoff, tf, tf*k/0.01, 5)
    csv_list = BuildcsvList(traj4, 1, csv_list)

    # MOVE TO FINAL STANDOFF POSITION
    Tse_standoff2 = Tsc_final@Tce_standoff
    traj5 = mr.ScrewTrajectory(Tse_standoff, Tse_standoff2, tf, tf*k/0.01, 5)
    csv_list = BuildcsvList(traj5, 1, csv_list)

    # STANDOFF2 ABOVE BLOCK DOWN TO BLOCK
    Tse_grasp2 = Tsc_final@Tce_grasp
    traj6 = mr.ScrewTrajectory(Tse_standoff2, Tse_grasp2, tf, tf*k/0.01, 5)
    csv_list = BuildcsvList(traj6, 1, csv_list)

    # RELEASE BLOCK
    traj7 = mr.ScrewTrajectory(Tse_grasp2, Tse_grasp2, int(tf/2), int((tf*k)/(2*0.01)), 5)
    csv_list = BuildcsvList(traj7, 0, csv_list)
    
    # MOVE BACK TO STANDOFF2 POSITION
    traj8 = mr.ScrewTrajectory(Tse_grasp2, Tse_standoff2, tf, tf*k/0.01, 5)
...
if __name__ == '__main__':
    ...
    # Run the generator
    csv_list = TrajectoryGenerator(Tse_initial, Tsc_initial, Tsc_final, Tce_grasp, Tce_standoff, k)

    np.savetxt('/home/codynichoson/milestone2_test.csv',csv_list, delimiter=',')
